...
async function seedMonitor(payload: Partial<MonitorRecord> = {}) {
  ...
  const monitor = {
    ...
    lastCertValidTo: payload.lastCertValidTo ?? null,
    lastCertDaysRemaining: payload.lastCertDaysRemaining ?? null,
    lastCertHostname: payload.lastCertHostname ?? null,
    lastSslStatus: payload.lastSslStatus ?? null,
    createdAt: now,
